...
#This code calculates the RMS frame-to-frame displacement in the y-direction as a function of the position in the box.  First, the displacement of the CM is fixed.  Then, the average y-direction velocity of each particle type and the total is computed in its bulk region. 
...
#Now, calculate the profile of velocities across the box
	...
	if typelist[i]==1 and xlist[i]>x and xlist[i]<x+cg:
	 ...
	 dispy[i]=ylist[i]-oldylist2[i]
         ...
         sumy2=sumy2+pow(dispy[i],2.0)
      
      vytype1[j]= vytype1[j] + math.sqrt(sumy1) #bin average velocity of each particle typ ein small slice of the lattive
      vytype2[j]= vytype2[j] + math.sqrt(sumy2)
      vytype1_std[j]=vytype1_std[j] + sumy1
      vytype2_std[j]=vytype2_std[j] + sumy2
      x=x+cg
...
for j in range(len(vytype1)):
 if countersum1[j]>0:
   vytype1[j]=vytype1[j]/countersum1[j]
   if vytype1_std[j]/countersum1[j]<pow(vytype1[j],2.0):
    print "type 1 error"
   vytype1_std[j]=math.sqrt((vytype1_std[j]/countersum1[j])-pow(vytype1[j],2.0))/math.sqrt(countersum1[j])
 if countersum2[j]>0:
   vytype2[j]=vytype2[j]/countersum2[j]
   if vytype2_std[j]/countersum2[j]<pow(vytype2[j],2.0):
    print "type 2 error"
   vytype2_std[j]=math.sqrt((vytype2_std[j]/countersum2[j])-pow(vytype2[j],2.0))/math.sqrt(countersum2[j])
