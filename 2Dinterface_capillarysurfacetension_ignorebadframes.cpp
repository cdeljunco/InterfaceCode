...
int main( int argc,char *argv[]){
    ...
    */
    ...
    while((fgets(line, sizeof(line), inputfile)) !=NULL){//looping through all frames until end of file
	    ...
	    if (loopj % 1 == 0){ //if (loopj % n == 0) -  do the following computation every n frames. Typically this frame selection happens in the lattice pre-processing (Script_analysis_latticeNew_1.py), in that case use 1.
            ...
            //The system should just have two clusters now. One of red and one of blue.
             ...
             cout << "Checking cluster analysis" << endl;
            
            //check whether the merged lattice is reasonable, defined by having a a solid line of filled lattice sites at the top or bottom. If not, skip this frame.
            ...
            for ( int x = 0; x < Lx; x++ ){
                ...
                cout << x << endl;
                cout << mylattice.nocc_image_processed[ x ][ 0 ][ 0 ] << endl;
                cout << mylattice.nocc_image_processed[ x ][ (Ly/2)-1 ][ 0 ] << endl;
                sum1 += mylattice.nocc_image_processed[ x ][ 0 ][ 0 ];
                sum2 += mylattice.nocc_image_processed[ x ][ (Ly/2)-1 ][ 0 ];

            }

            if (sum1 < Lx - 1 && sum2 < Lx - 1){
                
                cout << "Weird cluster analysis - skipping frame" << endl;
                weirdframes += 1;
                continue;
    ...
    fprintf( outputFourier, "Skipped %d frames\n", weirdframes); 
